...
DEFAULT_WANDB_SWEEP_NAME: Final[Optional[str]] = "Regressor A25 (Massa)" if DEFAULT_WANDB_SWEEP_ID is None else None
...
NETWORK_CHOICES: Dict[str, AnalyteClasses] = {
    ...
    **{name: AnalyteClasses(analyte=AnalyteName.SULFATE2D,  expected_range=(0.0, 4000.0),     network_class=obj, sample_dataset_class=sulfate2d.Sulfate2DSampleDataset,   processed_sample_dataset_class=sulfate2d.ProcessedSulfate2DSampleDataset)  for name, obj in inspect.getmembers(sulfate2d) if inspect.isclass(obj) and issubclass(obj, sulfate2d.Sulfate2DNetwork) and obj != sulfate2d.Sulfate2DNetwork},
    ...
    **{name: AnalyteClasses(analyte=AnalyteName.TOTAL_IRON, expected_range=(0.0, 20.0),         network_class=obj, sample_dataset_class=total_iron.TotalIronSampleDataset,  processed_sample_dataset_class=total_iron.ProcessedTotalIronSampleDataset)  for name, obj in inspect.getmembers(total_iron) if inspect.isclass(obj) and issubclass(obj, total_iron.TotalIronNetwork) and obj != total_iron.TotalIronNetwork},
    **{name: AnalyteClasses(analyte=AnalyteName.IRON_OXIDE,   expected_range=(0.0, 5.0), network_class=obj, sample_dataset_class=iron_oxide.IronOxideSampleDataset,     processed_sample_dataset_class=iron_oxide.ProcessedIronOxideSampleDataset)  for name, obj in inspect.getmembers(iron_oxide) if inspect.isclass(obj) and issubclass(obj, (iron_oxide.IronOxideNetwork, iron_oxide.IronOxideIntervalNetwork, iron_oxide.IronOxideBinaryNetwork)) and obj != iron_oxide.IronOxideNetwork and obj != iron_oxide.IronOxideIntervalNetwork and obj != iron_oxide.IronOxideBinaryNetwork},

}

DEFAULT_NETWORK_CLASS: Final[Type[Network]] = iron_oxide.IronOxideNetworkSqueezeNetStyle
...
DEFAULT_FIT_SAMPLES_BASE_DIRS: Final[Dict[str, List[str]]] = {
    ...
    AnalyteName.TOTAL_IRON: [
       os.path.join(DATASET_ROOT, "MABIDs-Dataset-TotalIron", "train")
    ],
    AnalyteName.IRON_OXIDE: [
       os.path.join(DATASET_ROOT, "MABIDs-Dataset-IronOxide", "train")
    ],
}
DEFAULT_TEST_SAMPLES_BASE_DIRS: Final[Dict[str, List[str]]] = {
    ...
    AnalyteName.TOTAL_IRON: [
        os.path.join(DATASET_ROOT, "MABIDs-Dataset-TotalIron", "test")
    ],
    AnalyteName.IRON_OXIDE: [
        os.path.join(DATASET_ROOT, "MABIDs-Dataset-IronOxide", "test")
    ],
}
...
DEFAULT_REDUCTION_LEVEL: Final[Dict[str, float]] = {
    ...
    AnalyteName.SULFATE2D: 0.10,
    AnalyteName.IRON2: 0.10,
    AnalyteName.IRON3: 0.10,
    AnalyteName.BISULFITE: 0.10,
    AnalyteName.PH: 0.10,
    AnalyteName.TOTAL_IRON: 0.10,
    AnalyteName.IRON_OXIDE: 0.10,
}
...
def main(args: Namespace) -> None:
    ...
    elif issubclass(args.net.network_class, IntervalNetwork) or issubclass(args.net.network_class, BinaryNetwork):
