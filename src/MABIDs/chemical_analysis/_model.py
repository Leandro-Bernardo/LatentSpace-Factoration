...
class BinaryNetwork(Network):
    def __init__(self, threshold: float, **kwargs: Any):
        super().__init__(**kwargs)
        expected_begin, expected_end = self.expected_range
        assert threshold > expected_begin and threshold < expected_end
        self.intervals = torch.as_tensor([[-float("Inf"), expected_begin], [expected_begin, threshold], [threshold, expected_end], [expected_end, float("Inf")]], dtype=torch.float32)
