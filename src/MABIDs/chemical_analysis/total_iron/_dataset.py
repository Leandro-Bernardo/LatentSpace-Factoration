...
class TotalIronSampleDataset(SampleDataset):
    ...
    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
        return [
            ...
            self._parse_auxiliary_solution("Oxidant", raw_sample["oxidant"]),
        ]

    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        return (
            raw_sample["sourceStock"].get("totalironConcentration", None),
            raw_sample.get("estimatedConcentration", None),
            raw_sample["sourceStock"].get("totalironConcentrationUnit", "MILLIGRAM_PER_LITER_OF_TOTAL_IRON"),
        )


class ProcessedTotalIronSampleDataset(ProcessedSampleDataset):
    ...
    def _whitebalance_type(self):
        return WhiteBalanceType.USING_GRID
