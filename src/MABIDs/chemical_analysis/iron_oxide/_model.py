from .._model import *
from ..typing import CalibratedDistributions, Distribution, Prediction, ExtractedFeatures, Logits
...
class IronOxideNetwork(ContinuousNetwork):
...
class IronOxideUpNetwork(ContinuousUpNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.model = torch.nn.Sequential(OrderedDict([
            ("backbone", backbone),
...
class IronOxideIntervalNetwork(IntervalNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.model = torch.nn.Sequential(OrderedDict([
            ("pre_processing", PreProcessing(**kwargs)),
...
class IronOxideIntervalNetworkSqueezeNetStyle(IronOxideIntervalNetwork):
...
class IronOxideBinaryNetwork(BinaryNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.model = torch.nn.Sequential(OrderedDict([
            ("pre_processing", PreProcessing(**kwargs)),
...
class IronOxideBinaryNetworkSqueezeNetStyle(IronOxideBinaryNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=SqueezeNet1_1(in_channels=1, num_classes=4), threshold=1.0, **kwargs)
...
class PostProcessing(Module):
    ...
    def forward(self, normalized_iron_oxide: torch.Tensor) -> Prediction:
        normalized_iron_oxide = normalized_iron_oxide.squeeze(-1)
        iron_oxide = (2.0 * normalized_iron_oxide - 1.0) * (2.5758 * 1.4826 * self.training_mad) + self.training_median
        return iron_oxide, normalized_iron_oxide
...
class IronOxideEstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(IronOxideNetwork, **kwargs)
