...
from ..typing import AuxiliarySolution, ChamberType, WhiteBalanceType
from ._utils import compute_masks, compute_pmf
from typing import Any, Dict, List, Optional, Tuple
import numpy as np


class IronOxideSampleDataset(SampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
        return [
            self._parse_auxiliary_solution("Reagente", raw_sample["INTERNAL_USE"]),
        ]
    
    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        # mass = raw_sample["sourceStock"].get("ironOxideConcentration", None)
        # volume = raw_sample["extraValues"].get("FilteredVolume", None)
        # if mass == 0.0:
        #     return (float(mass), 0.0, raw_sample["sourceStock"].get("ironOxideConcentrationUnit", None))
        # if volume <= 0.0:
        #     raise 'Volume can not be zero or negative!'
        # estimated_concentration = mass/(volume/1000)
        return (raw_sample["sourceStock"].get("ironOxideConcentration", None), None, raw_sample["sourceStock"].get("ironOxideConcentrationUnit", None))

class ProcessedIronOxideSampleDataset(ProcessedSampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
    ...
    def _whitebalance_type(self):
        return WhiteBalanceType.EXCEPT_ANALYTE
