...
from ._segmentation import load_model, generate_masks, resize_masks
...
def whitebalance_2(bgrs: torch.Tensor, bright_masks: torch.Tensor, analyte_masks: torch.Tensor, threshold: Optional[int] = 250) -> torch.Tensor:
    '''
    Args:
        bgrs (torch.Tensor): a tensor of shape [b,c,h,w] corresponding to the unbalanced images
        bright_masks (torch.Tensor): a tensor of shape [b,h,w] corresponding to the computed bright mask
        analyte_masks (torch.Tensor): a tensor of shape [b,h,w] corresponding to the computed analyte mask
        threshold (Optional[int]): a optional value to use as threshold for the dark channel
    Returns:
        torch.Tensor: containing the images with the white balance applyed
    '''
    # dark channel
    dc = torch.min(bgrs, dim=1, keepdim=True)[0]
    # constant light source
    a = (bgrs.sum(dim=1, keepdim=True)/3).mean(dim=1, keepdim=True) # a can be zero
    # transition map
    t_map = 1 - (dc/torch.maximum(a, torch.ones([], device=a.device, dtype=a.dtype)[None,None,None,None])) # when a is zero an apsilon should be chosen?
    avg_t_map = t_map.mean(dim=(-2,-1))
    # white regions calculation
    white_region = ~(bright_masks | analyte_masks)[:,None,:,:] & (t_map <= avg_t_map[:,:,None,None]) & (dc<threshold)
    # compute BGR gain
    bgr_gain = torch.sum(bgrs*white_region, dim=(2,3))/(torch.sum(white_region, dim=(2,3))*255)
    # color constancy algorithm
    Wy = torch.sum(bgr_gain*torch.tensor([0.0722, 0.7152, 0.2126], device=bgr_gain.device), dim=1)[...,None] # if results are wrong, verify if it is in columns or rows
    img_result = torch.clamp(((bgrs*Wy[...,None,None])/bgr_gain[...,None,None]), max=255).type(bgrs.dtype)
    return img_result
...
def lab_to_normalized(lab: np.ndarray, *, lab_white: np.ndarray, lab_mean: Optional[np.ndarray] = None, lab_sorted_eigenvectors: Optional[np.ndarray] = None, out_channels: Tuple[int, ...]) -> np.ndarray:
    # TODO utilizar de um método abstrato para informar o tipod e whitebalance a ser utilizado.
...
def _compute_masks_for_membrane(bgr_img: np.ndarray, lab_img: Optional[np.ndarray]) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], np.ndarray, np.ndarray]:
    # Convert the input image from BGR to L*a*b* and set the correct range for the channels, i.e., 0 ≤ L* ≤ 100, −127 ≤ a* ≤ 127, −127 ≤ b* ≤ 127
    if lab_img is None:
        lab_img = bgr_to_lab(bgr_img)
    # Resize the input image
    resized_height, resized_width = 1024, 512 # TODO colocar para pegar o valor via biblioteca de segmentação
    resized_lab_img = cv2.resize(lab_img, (resized_width, resized_height), interpolation=cv2.INTER_CUBIC)
    # Load the segmentation model and compute the masks
    model = load_model()
    trans_bgr_img = np.transpose(bgr_img, (2,0,1))
    resized_bgr = T.Resize(size=(1024, 512))(as_tensor(trans_bgr_img, dtype=float32).to(next(model.parameters()).device)) # TODO colocar para pegar o valor do resize do modelo carregado
    bg_msk, grid_msk, _, analyte_msk  = generate_masks(model=model, image=resized_bgr) # On version 0.1.0 of mabdis_seg the masks are 0: background, 1: grid, 2: analyte_holder, 3: analyte
    # Preventing analyte_msk from been empty by extimating it from the grid mask
    if analyte_msk.max() == False:
        msk_ = grid_msk.detach().cpu().numpy().astype(np.uint8)
        msk_ = cv2.GaussianBlur(msk_, (9,9), 2)
        kernel_size = (9,9)
        circular_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, kernel_size)
        eroded_msk = cv2.erode(msk_, circular_kernel, iterations=1)
        coords = np.where(eroded_msk)
        centroid_x = int(np.mean(coords[1]))
        centroid_y = int(np.mean(coords[0]))
        size_x = int(coords[1].max() - coords[1].min())
        size_y = int(coords[0].max() - coords[0].min())
        new_msk = cv2.circle(eroded_msk, (centroid_x, centroid_y), min(size_x, size_y)//4, (255, 255, 255), thickness=-1)
        new_msk = torch.from_numpy(new_msk).float().to(grid_msk.device)
        new_msk /= new_msk.max()
        analyte_msk = new_msk
    grid_bw = grid_msk[..., None].detach().cpu().numpy().copy()
    _, grid_bw = cv2.threshold(((grid_bw.astype(np.float32) / 255.0).prod(axis=2) * 255.0).astype(np.uint8), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # Compute the reference L*a*b value for white as the median L*a*b* values among the grid pixels
    lab_white = np.median(resized_lab_img[grid_bw != 0, :], axis=0)
    # Combining background and grid masks to use as brigth mask. In the new white balance que bright mask is really every mask that not include the mambrane
    no_mambrane_msk = (bg_msk+grid_msk)
    # Resize the masks to the orinal input size
    grid_msk = resize_masks(grid_msk[None,...].detach().cpu(), bgr_img.shape[:-1]).squeeze(0).numpy().astype(np.bool_)
    analyte_msk = resize_masks(analyte_msk[None,...].detach().cpu(), bgr_img.shape[:-1]).squeeze(0).numpy().astype(np.bool_)
    no_mambrane_msk = resize_masks(no_mambrane_msk[None,...].detach().cpu(), bgr_img.shape[:-1]).squeeze(0).numpy().astype(np.bool_)

    # *************************************
    # Test if the results are not good enough
    # TODO utilizar a máscara de analito para calcular o maior blob e fazer sequência de erosão igual ao realizado no relatório
    # TODO considerar a máscara original do analito como área para delimitar o white balance 2
    # *************************************
    
    # Return the masks (bright, grid, analyte, and attention), the L*a*b* version of the image, and the reference L*a*b value for white
    return (no_mambrane_msk, grid_msk, analyte_msk), lab_img, lab_white
...
def _compute_masks(bgr_img: np.ndarray, lab_img: Optional[np.ndarray], chamber_type: ChamberType, min_bright_threshould: Optional[int]) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], np.ndarray, np.ndarray]:
    ...
    elif chamber_type is ChamberType.MEMBRANE:
        return _compute_masks_for_membrane(bgr_img, lab_img)
    else:
