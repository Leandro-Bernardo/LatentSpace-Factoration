...
class DataModule(LightningDataModule):
    ...
    def _compute_pca_stats(self, subset: ProcessedSampleDataset) -> Dict[str, np.ndarray]:
        ...
        match subset._whitebalance_type():
                ...
                case WhiteBalanceType.EXCEPT_ANALYTE:
                    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                    # Create indexes slices
                    down_idxs = [x for x in range(0, len(subset), self.batch_size)]
                    upper_idxs = [x for x in range(self.batch_size, len(subset), self.batch_size)]
                    if len(down_idxs) > len(upper_idxs):
                        upper_idxs.append(len(subset)-1)
                    idxs_slices = [(down, up) for down, up in zip(down_idxs, upper_idxs)]
                    # Defining function to set labs inside a map
                    def set_labs(idx: int, lab: np.ndarray):
                        labs[:, idx] = lab.sum(axis=0)
                    # Iterating over the dataset in batches and applying white balance 2
                    for idxs in tqdm(idxs_slices, total=len(idxs_slices), desc="Computing PCA statistics", leave=False):
                        down, upper = idxs
                        subset_slice = list(map(lambda idx, item: (idx, item.sample_lab_image.shape, item), range(down, upper), subset[down:upper]))
                        # Iterating over diferent shapes in the batch
                        for shp in list(set(map(lambda item: item[1], subset_slice))):
                            # Selecting only the corresponding shape
                            subset_slice_shaped = list(filter(lambda x: x[1] == shp, subset_slice))
                            # Loading data as np.ndarrays
                            bgr = np.stack(list(map(lambda item: lab_to_bgr(item[2].sample_lab_image), subset_slice_shaped)), axis=0).transpose((0, 3, 1, 2))
                            bright_masks = np.stack(list(map(lambda item: item[2].sample_bright_mask, subset_slice_shaped)), axis=0)
                            ...
                            lab = list(map(lambda idx: bgr_to_lab(bgr[idx,...]), range(len(subset_slice_shaped))))
                            lab = list(map(lambda idx: lab[idx][analyte_masks[idx]], range(len(lab))))
                            list(map(lambda idx: set_labs(subset_slice_shaped[idx][0], lab[idx]), range(len(lab))))
                case _:
    ...
    def _compute_pca_stats_float64(self, subset: ProcessedSampleDataset) -> Dict[str, np.ndarray]:
        labs = list()
        for index, item in enumerate(tqdm(iter(subset), total=len(subset), desc="Computing PCA statistics", leave=False)):
            analyte_msk = item.sample_analyte_mask
            lab = whitebalance(item.sample_lab_image[analyte_msk].astype(np.float64), item.sample_lab_white.astype(np.float64))
            labs.append(lab)
        labs = np.concatenate(labs, axis=0, dtype = np.float64)
        mean = labs.mean(axis=0, keepdims=True)
        eigenvalues, eigenvectors = np.linalg.eig(np.cov(labs - mean, rowvar=False))
        sorted_indices = np.argsort(np.abs(eigenvalues))[::-1]
        sorted_eigenvalues = eigenvalues[sorted_indices]
        sorted_eigenvectors = eigenvectors[:, sorted_indices]
        # returns float 32
        return {"lab_mean": mean.squeeze(axis=0).astype(np.float32), "lab_sorted_eigenvalues": sorted_eigenvalues.astype(np.float32), "lab_sorted_eigenvectors": sorted_eigenvectors.astype(np.float32)}
...
class BaseModel(ABC, LightningModule):
    ...
    def log_interval_inferences(self, mode):
        ...
        targets = self.inf_vs_target[mode][0]['target'].cpu().numpy().astype(np.uint8)
        labels = list(map(lambda item: f"({item[0]}, {item[1]})", self.net.intervals))
        # Confusion Matrix
        cm = confusion_matrix(targets, predictions, labels=np.arange(len(labels)))
        fig = px.imshow(
            cm,
            x=labels,
            y=labels,
            ...
            labels=dict(x="Classe Predita", y="Classe Real", color="Contagem"),
            title=f"Matriz de Confusão ({mode})"
        )
        fig.update_layout(
            title_x=0.5,
        ...
        wandb.log({f"Matriz de Confusao/{mode}": fig})

    def log_continuous_inferences(self, mode):
...
class IntervalModel(BaseModel):
    ...
    def _any_step(self, batch: Tuple[CalibratedDistributions, Values], mode_name: str) -> Loss:
        ...
        with torch.no_grad():
            self.outputs[mode_name].append({
                "predicted_value": logits.detach().argmax(-1),
                "target_value": target,
            })
        # Return the loss value.
        return loss
    ...
    @classmethod
    def wandb_metric(cls) -> Dict[str, Any]:
        return {"name": "F1-Score/Val/Epoch", "goal": "maximize"}
